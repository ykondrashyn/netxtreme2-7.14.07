...
MODULE_PARM_DESC(debug_logging, " Enable more messaging from the driver. "
	"Nominal debug level is 0xffff.");
...
uint bnx2fx_tm_timeout = BNX2FC_TM_TIMEOUT;
module_param_named(tm_timeout, bnx2fx_tm_timeout, uint, S_IRUGO|S_IWUSR);
...
#else
		devst = per_cpu_ptr(lport->stats, cpu);
#endif
...
#if defined(__BNX2FC_SLES__) &&  __BNX2FC_SLES__ > 0x1201
...
static void bnx2fc_recv_frame(struct sk_buff *skb)
{
	...
	if (lport->port_id != ntoh24(fh->fh_d_id)) {
...
static void bnx2fc_ulp_init(struct cnic_dev *dev)
{
	...
	printk(KERN_ERR PFX "FCoE initialized for %s.\n",
	    dev->netdev->name);
...
static struct scsi_host_template bnx2fc_shost_template = {
	...
	.lockless		= 1,
#endif
#if (!defined(__BNX2FC_UEK__) && !defined(__BNX2FC_CITRIX__) && (defined(__BNX2FC_RHEL__) && (__BNX2FC_RHEL__ >= 0x0700)))
	.no_async_abort		= 1,
#endif
